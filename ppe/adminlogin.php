...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($logMail_err) && empty($logMDP_err)) {
        ...
        $sql = "SELECT uID, uLogin, uMDP FROM utilisateur WHERE uLogin = ? AND uAdmin = 1;";
        ...
        if ($stmt = $mysqli->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt->num_rows == 1) {
                    ...
                    echo $logMail_err = ('<div style="position: absolute; bottom: 2px;">Ce compte n\'a pas les droits d\'administrateur.</div>');
                }
            } else {
